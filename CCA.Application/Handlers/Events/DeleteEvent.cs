using CCA.Models.Requests;
using CCA.Models.Validators;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Azure.Cosmos.Table;
using Microsoft.Extensions.Logging;
using System.Linq;
using System.Threading.Tasks;
using EventDto = CCA.Models.Storage.Event;

namespace CCA.Application.Handlers.Events
{
    public class DeleteEvent
    {
        private readonly DeleteEventDto _deleteEventRequest;
        private readonly CloudTable _eventsTable;
        private readonly string _loginUserId;
        private readonly ILogger _logger;

        public DeleteEvent(
            DeleteEventDto deleteEventRequest,
            CloudTable eventsTable,
            string loginUserId,
            ILogger logger)
        {
            _deleteEventRequest = deleteEventRequest;
            _loginUserId = loginUserId;
            _eventsTable = eventsTable;
            _logger = logger;
        }

        public async Task<IActionResult> ProcessAsync()
        {
            await _eventsTable.CreateIfNotExistsAsync();

            try
            {
                Validator.ValidateRequest(_deleteEventRequest);

                var events = _eventsTable
                    .CreateQuery<EventDto>()
                    .AsQueryable()
                    .Where(e => e.RowKey == _deleteEventRequest.Id)
                    .ToArray();

                if (events.Length == 0 || events[0].UserId != _loginUserId)
                {
                    return new NotFoundResult();
                }

                var @event = events[0];

                await _eventsTable.ExecuteAsync(TableOperation.Delete(@event));

                _logger.LogInformation("Deleted event with id: {0}", @event.RowKey);

                return new NoContentResult();
            }
            catch (ValidationException e)
            {
                return new BadRequestObjectResult(new { errors = e.Message });
            }
        }
    }
}
